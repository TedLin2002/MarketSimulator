...
export function createState(seed = 12345) { return { day:0, seed:Number(seed), listings:structuredClone(initialListings), logs:['市場已準備完成：NPC 賣家已上架 6 項商品。'], events:[], history:[], pendingActions:[] }; }
...
export function validateAction(action, state) {
  if (!action || typeof action !== 'object' || !['list_product','update_price','restock','delist_product'].includes(action.type)) return '未知的動作類型';

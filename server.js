...
import { createState, publicState, queueActions, step, buyers, info } from './src/engine.js';
...
async function getBuyerDecisions(currentState) {
  const listings = currentState.listings.filter(listing => listing.inventory > 0).map(listing => ({ id:listing.id, product:info[listing.productId].name, category:info[listing.productId].category, price:listing.price, rating:Number(listing.rating.toFixed(1)), inventory:listing.inventory }));
  const prompt = `你正在扮演電商平台上的五位買家。今天是第 ${currentState.day + 1} 天。每位買家最多買一件，也可以不買。只從提供的 listingId 中選擇；依預算、喜好、價格與評分做自然決定。buy 為 false 時仍提供 review 但 text 留空。buy 為 true 時，寫一則繁體中文、30 字內的真實短評，score 是 1 到 5。\n買家：${JSON.stringify(buyers)}\n商品：${JSON.stringify(listings)}`;
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/step') { const days = Math.min(365, Math.max(1, Number(body.days) || 1)); for (let day = 0; day < days; day++) state = step(state, openaiApiKey ? await getBuyerDecisions(state) : null); return send(res, 200, publicState(state)); }
